...
router.post('/', async (req, res) => {
    try {
        ...
        // If payment method is cash, deduct from Stock.cashInHand
        if (paymentMethod === 'cash') {
            let stock = await Stock.findOne({ userId: req.userId });
            if (!stock) {
                stock = await Stock.create({ userId: req.userId, gold: 0, silver: 0, cashInHand: 0 });
            }
            stock.cashInHand = (stock.cashInHand || 0) - parseFloat(amount);
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        // If payment method was cash, add back to Stock.cashInHand
        if (expense.paymentMethod === 'cash') {
            let stock = await Stock.findOne({ userId: req.userId });
            if (!stock) {
                stock = await Stock.create({ userId: req.userId, gold: 0, silver: 0, cashInHand: 0 });
            }
            stock.cashInHand = (stock.cashInHand || 0) + parseFloat(expense.amount);
            stock.updatedAt = new Date();
            await stock.save();
        }
